...
(3)函数执行完毕，即已经执行了中间某处，或者函数尾部的return语句后，不可再恢复，因为函数体已走完。
...
(1)当前函数的返回地址.当前函数如果返回到上一级函数后，继续执行指令的地址.该地址在上一级调用当前函数时自动存入LR寄存器。

(2)当前函数保护的寄存器.当前函数在执行过程中将会覆写大量寄存器，如果不保护的话返回到上一级函数后，上一级函数数据就混乱.尤其需要保护上一级函数的桢指针fp.保护方式为将寄存器依次入栈，当前函数返回前再一次出栈.
...
反之，函数返回后，栈空间将向上收缩，收缩过程与如上的伸展过程完全相反.而当再次发生函数调用，栈空间又会再次伸展,如此便是函数调用的基本过程，栈空间可以认为是函数活动记录的重要数据区.
