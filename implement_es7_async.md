...
##### (2)协程机制
协程的概念与原理资料颇多，总结来说，协程可以理解为可中止(中间停止,非终止)与恢复的函数。此处的中止与恢复并不是线程的挂起与恢复.假设当前函数A中止执行了，线程不会挂起，而是转而去执行函数B了，B函数也可能中止执行，然后又恢复执行函数A.就像cpu时间片轮转一样，一个线程“同时”执行着好几个函数，这种函数叫做“协程”。

此处如果运用协程机制，那么await中止而不挂起线程，异步操作后在意某种机制恢复await的执行，正是想要的效果.
